################################################################
# Create dynamic reports using Serenity and the shinyAce editor
################################################################
...
This is an example of the type of report you can write in Serenity.
...
It is also possible to generate interactive tables using the DT package. In Serenity you can use the `dtab` function to display a data.frame as a nicely formatted table:
...
> Note that adding `%>% render` to display the table is not required when using `dtab` outside of Serenity

### Documenting analysis results in Serenity

The report feature in Serenity should be used in conjunction with the <i title='Report results' class='fa fa-edit'></i> icons shown at the bottom of the side bar on (almost) all pages. When that icon is clicked the command used to create the ouput is copied into the editor in the _R > Report_ tab. By default Serenity will paste the code generated for the analysis you just completed at the bottom of the report (i.e., `Auto paste`). However, you can turn off that feature by selecting `Manual paste` from the dropown. With manual paste on, the code is put in the clipboard when you click a report icon and you can paste it where you want in the _R > Report_ editor window.
...
Below is some code generated by Serenity to produce a scatterplot / heatmap of the price of diamonds versus carats. The colors in the plot reflect the clarity of the diamond.
    ...
    with(tags,
      table(
        td(help_modal('Report','report_help',
           inclMD(file.path(getOption("serenity.path.data"),"app/tools/help/report.md")))),
        td(HTML("&nbsp;&nbsp;")),
...
knitItSave <- function(text) {
  ...
  markdown::markdownToHTML(text = gsub("\\\\\\\\","\\\\",preserved$value),
                           ...
                           stylesheet = file.path(getOption("serenity.path.data"),"app/www/bootstrap.min.css")) %>%
  htmltools::restorePreserveChunks(preserved$chunks)
}

## Knit for report in Serenity
knitIt <- function(text) {
  ...
  ret <- paste(markdown::markdownToHTML(text = md, fragment.only = TRUE, stylesheet = ""),
        paste0("<script type='text/javascript' src='", getOption("serenity.mathjax.path"),"/MathJax.js?config=TeX-AMS-MML_HTMLorMML'></script>"),
...
## make all required libraries available by loading serenity package if needed
          ...
          withProgress(message = paste0("Saving report to ", input$rmd_save_report), value = 1,
            if (rstudioapi::isAvailable() || !isTRUE(local)) {
              ...
              out <- rmarkdown::render("report.Rmd", switch(input$rmd_save_report,
                ...
                Word = rmarkdown::word_document(reference_docx = file.path(system.file(package = "serenity.data"),"app/www/style.docx"))
              ), envir = r_environment)
...
update_report_fun <- function(cmd) {
  isolate({
    ...
    if (state_init("rmd_switch", "Switch tab") == "Switch tab")
      updateTabsetPanel(session, "nav_serenity", selected = "Report")
  })
}
